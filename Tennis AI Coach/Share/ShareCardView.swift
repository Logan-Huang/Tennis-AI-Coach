...
/// Single-swing variant: the frame is the hero, score ring composited on it.
struct ShotShareCard: View {
    let shot: ShotScore
    let stroke: Stroke
    let frame: UIImage?
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            if let frame {
                ...
                Rectangle().fill(Color(red: 0.10, green: 0.44, blue: 0.30))
            }
            ...
            HStack(alignment: .top) {
                ...
                ZStack {
                    ...
                    Circle()
                        .trim(from: 0, to: shot.overall.isFinite ? max(0.02, shot.overall / 100) : 0)
                        .stroke(.white, style: StrokeStyle(lineWidth: 12, lineCap: .round))
                        .rotationEffect(.degrees(-90))
                        .padding(10)
                    Text(Fmt.score(shot.overall))
